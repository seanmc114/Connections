...
(() => {
  ...
  // Unlock targets (need previous level best <= target)
  const BASE_THRESH = { 1:200, 2:180, 3:160, 4:140, 5:120, 6:100, 7:80, 8:60, 9:40 };

  // Spanish "reads" tokens (global pool)
  const GLOBAL_READS_MAX = 7;
  const GLOBAL_READS_KEY = "turboConnectors:globalReads:CONNECTIONS_BRIGHT1";

  // Local best storage
  const STORAGE_PREFIX = "turboConnectors:CONNECTIONS_BRIGHT1";
  ...
  function matchFinger(matchCode){
    const h = simpleHash32((matchCode || "") + "|MF|CONNECTIONS_BRIGHT1");
    const v = h % (36*36);
    return b36pad(v, 2);
  }
  function makeShortResultCode({ lvl, mode, match, score, correct, wrong }){
    ...
    const sigNum = simpleHash32(body + "|SIG|TURBO_CONNECTIONS_BRIGHT1") % (36*36);
